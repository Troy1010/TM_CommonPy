...
class CommandSet:
    ...
    def Que(self, cDoUndoPair, cArgs):
        ...
        if len(cDoUndoPair) != 2:
            raise ValueError(self.__class__.__name__ + "::" + TM.FnName()
                             + "`first arg must be a container of 2 methods: Do and Undo")
        if not TM.IsCollection(cArgs):
            cArgs = [cArgs]
        # ---
        self.CommandSet.append(
            [CommandSet_QueType.Function, cDoUndoPair, cArgs])
    ...
    def QueScript(self, sFilePath, cArgs):
        ...
        vModule = ModuleType("CommandSetQuedScript",
                             "This module represents a qued script")
        exec(sScript, vModule.__dict__)
        if not (hasattr(vModule, "Do") and hasattr(vModule, "Undo")):
            raise Exception(
                "QueScript`Script must have Do and Undo functions.")
        # ---
    ...
    def Execute(self, bRedo=False):
        ...
        if bRedo:
            ...
            for vItem in self.PreviousCommandSet:
                TMLog.debug(self.__class__.__name__ + "::"
                            + TM.FnName() + "`Reundo:" + str(vItem[0]))
                self._Undo(*vItem)
        else:
            ...
            for vItem in [x for x in self.PreviousCommandSet if x not in self.CommandSet]:
                TMLog.debug(self.__class__.__name__ + "::"
                            + TM.FnName() + "`Undo:" + str(vItem[0]))
                self._Undo(*vItem)
        if bRedo:
            # ---Do what is in CommandSet
            for vItem in self.CommandSet:
                TMLog.debug(self.__class__.__name__ + "::"
                            + TM.FnName() + "`Redo:" + str(vItem[0]))
                self._Do(*vItem)
        else:
